...
import {cloudStorage, firestoreDb} from './firebaseConfiguration';
...
const saveSongFile = (userId, docRefId, songFile) => {
    ...
    uploadTask.on('state-changed',
        ...
        function error(error) {
            console.log('There was an error while saving to Cloud Storage: ', error);
        },
        ...
        firebase.auth().onAuthStateChanged((user) => {
            if (user) {
                ...
                songDocument.get()
                    ...
                    .catch((error) => {
                        console.error(`There was an error while trying to get song with id ${songId}`);
                        resolve();
                    });
            }
        })
...
export const getArtistName = (userId) => {
    return new Promise((resolve) => {
        // Get reference to the user docuent
